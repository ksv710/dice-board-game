...
void testHelperStartup(const string &name, int compile_marks, int total_marks) {
  assert(name != "");
...
bool testHelperIsCapturing() { return ptr_capture_old_puffer != NULL; }
...
void testHelperPrintSummaryHeader(unsigned int indent, unsigned int max_digits,
...
void testHelperPrintSummaryLine(const std::string &text, bool is_correct) {
  assert(text != "");
...
void testHelperPrintSummaryLine(const std::string &text, unsigned int achieved,
...
void testHelperPrintSummaryLine(const std::string &text, double percentage) {
  assert(text != "");
...
unsigned int testHelperMarkAnyAll(unsigned int correct, unsigned int maximum) {
  assert(maximum >= 2);
...
void testHelperPrintMark(unsigned int achieved, unsigned int maximum) {
  assert(maximum >= 1);
  assert(achieved <= maximum);
